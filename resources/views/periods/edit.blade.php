<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ __('Edit Cycle') }}</h2>
    </x-slot>

    <div class="py-12">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow-sm sm:rounded-lg p-6">
                <form action="{{ route('periods.update', $period) }}" method="POST" class="space-y-4">
                    @csrf @method('PUT')
                    <div>
                    ...
                    <div>
                        ...
                        <input type="text" name="symptoms" class="w-full border-gray-300 rounded-md shadow-sm"
                            value="{{ old('symptoms', $period->symptoms) }}">
                    </div>
                    <div>
                        <label class="block text-gray-700 font-bold mb-2">Notes</label>
                        <textarea name="notes" rows="4"
                            class="w-full border-gray-300 rounded-md shadow-sm">{{ old('notes', $period->notes) }}</textarea>
                    </div>
                    <div class="flex items-center space-x-4">
                        <button type="submit"
                            class="bg-pink-600 text-white px-4 py-2 rounded shadow hover:bg-pink-700">Update
                            Log</button>
